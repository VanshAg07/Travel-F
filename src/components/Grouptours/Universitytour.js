    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "https://api.travelo10.com/api/group-tours/group-tours"
        );
                ...
                {item.stateImage &&
                  item.stateImage.map((imgUrl, index) =>
                    imgUrl.endsWith(".mp4") ? (
                      ...
                      >
                        <source
                          src={`https://api.travelo10.com/upload/${imgUrl}`}
                          type="video/mp4"
                        />
        ...
        <div className="w-[95%]">
          {adventureGroupStart.length > 0 &&
            adventureGroupStart.map((item) => (
              <div
                key={item._id}
                className="flex flex-col md:flex-row items-start bg-white rounded-lg overflow-hidden"
              >

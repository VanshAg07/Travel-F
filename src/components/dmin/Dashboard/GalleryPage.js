...
function GalleryPage() {
  ...
  const fetchImages = async () => {
    ...
    try {
      const res = await axios.get(
        "https://api.travelo10.com/api/home/gallery-page"
      );
      setImages(res.data);
    } catch (error) {
  ...
  const deleteImage = async (id, imagePath) => {
    ...
    try {
      await axios.delete(
        `https://api.travelo10.com/api/home/gallery-page/delete-image/${id}/${encodeURIComponent(
          imagePath
        )}`
      );
      fetchImages();
              ...
              <div className="grid gap-4">
                {gallery.images.map((imgPath, idx) => (
                  <div key={idx} className="relative">
                    <img
                      src={`https://api.travelo10.com/${imgPath}`}
                      alt="Gallery"
      ...
      {showModal && (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center">
          <div className="bg-white p-4 rounded-lg shadow-lg relative">
            <img
              src={`https://api.travelo10.com/${currentImage}`}
              alt="Current"

...
function PackagesVideo() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editing) {
      await axios.put(
        `http://localhost:5000/api/home/video-page/${editing}`,
        data
      );
      setEditing(null);
    } else {
      await axios.post("http://localhost:5000/api/home/video-page", data);
    }
  ...
  const handleDelete = async (id) => {
    await axios.delete(`http://localhost:5000/api/home/video-page/${id}`);
    fetchVideoPages();
  };

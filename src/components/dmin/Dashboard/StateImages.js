...
function StateImages() {
  ...
  const fetchSignIns = async () => {
    try {
      const response = await axios.get(
        "https://api.travelo10.com/api/popup/state-images"
      );
  ...
  const updateSignIn = async () => {
    ...
    try {
      const response = await axios.put(
        `https://api.travelo10.com/api/popup/state-images/${selectedSignIn._id}`,
        formData,
        { headers: { "Content-Type": "multipart/form-data" } }
      );
  ...
  const deleteSignIn = async (id) => {
    try {
      await axios.delete(`https://api.travelo10.com/api/popup/state-images/${id}`);

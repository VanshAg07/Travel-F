...
const FooterIconsManager = () => {
  ...
  const fetchIcons = async () => {
    try {
      const response = await axios.get(
        "https://api.travelo10.com/api/home/footer-icons"
      );
  ...
  const handleEditIcon = async (id) => {
    ...
    try {
      await axios.put(
        `https://api.travelo10.com/api/home/footer-icons/${id}`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  const handleDeleteIcon = async (id) => {
    try {
      await axios.delete(`https://api.travelo10.com/api/home/footer-icons/${id}`);
      fetchIcons();
    } catch (error) {

...
function CorporateHome() {
  ...
  const fetchCorporates = async () => {
    try {
      const response = await axios.get(
        "https://api.travelo10.com/api/corporate/create-corporate-landing"
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(
          `https://api.travelo10.com/api/corporate/update-corporate-landing/${editingId}`,
          formData
        );
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(
        `https://api.travelo10.com/api/corporate/delete-corporate-landing/${id}`
      );

...
function HomeHoneymoon() {
  ...
  const fetchInternationalData = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/api/home/homepage-honeymoon"
      );
  ...
  const handleImageChange = (event) => {
    setNewEntry({ ...newEntry, image: Array.from(event.target.files) });
  };
  ...
  const handleSaveEntry = async (event) => {
    ...
    // Append the new images, or keep existing images when editing
    if (editing) {
      newEntry.image.forEach((img) => formData.append("image", img));
    } else {
      newEntry.image.forEach((img) => formData.append("image", img));
    }
    ...
    try {
      if (editing) {
        await axios.put(
          `http://localhost:5000/api/home/homepage-honeymoon/${editing._id}`,
          formData,
          {
  ...
  const handleDeleteEntry = async (id) => {
    try {
      await axios.delete(
        `http://localhost:5000/api/home/homepage-honeymoon/${id}`
      );
      fetchInternationalData();
    } catch (error) {

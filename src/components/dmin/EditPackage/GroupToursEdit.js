...
function GroupToursEdit() {
  ...
  useEffect(() => {
    axios
      .get(
        "https://api.travelo10.com/api/group-tours/group-tours-edit"
      )
      .then((response) => {
        if (response.data) {
  ...
  const handleSubmit = (e) => {
    ...
    if (selectedTrip) {
      ...
      axios
        .put(
          `https://api.travelo10.com/api/group-tours/edit-group-tour-package/${selectedTrip.stateName}/${selectedTrip._id}`,
  ...
  const handleDeleteTrip = (pkg, tripId) => {
    ...
    if (confirmed) {
      axios
        .delete(
          `https://api.travelo10.com/api/group-tours/delete-group-tour-package/${pkg.stateName}/${tripId}`
        )
        .then((response) => {

...
const BlogManagement = () => {
    ...
    const fetchBlogs = async () => {
      try {
        const response = await axios.get(
          "https://api.travelo10.com/api/blog/blogs"
        );
  ...
  const updateBlog = async () => {
    try {
      await axios.put(
        `https://api.travelo10.com/api/blog/blogs/${selectedBlog._id}`,
        selectedBlog
      );
      ...
      // Refresh blogs
      const response = await axios.get("https://api.travelo10.com/api/blog/blogs");
      setBlogs(response.data.blogs);
    } catch (error) {
  ...
  const deleteBlog = async (blogId) => {
    if (window.confirm("Are you sure you want to delete this blog?")) {
      try {
        await axios.delete(`https://api.travelo10.com/api/blog/blogs/${blogId}`);

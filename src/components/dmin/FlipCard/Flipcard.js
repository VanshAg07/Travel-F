...
const Flipcard = () => {
  ...
  const fetchFlipcards = async () => {
    try {
      const response = await axios.get(
        "https://api.travelo10.com/api/flip-card/flip"
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing && editFlipcardId) {
        ...
        await axios.put(
          `https://api.travelo10.com/api/flip-card/flip/${selectedCategory}/${editFlipcardId}`,
          formData
        );
        setIsEditing(false);
        setEditFlipcardId(null);
      } else {
        await axios.post("https://api.travelo10.com/api/flip-card/flip", formData);
      }
  ...
  const handleDelete = async (category, stateName) => {
    ...
    try {
      ...
      await axios.delete(`https://api.travelo10.com/api/flip-card/flip`, {
        data: {
          category: category,
          stateName: stateName,
        },

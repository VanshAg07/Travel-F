...
function AddWeekend() {
  ...
  const fetchNationalData = async () => {
    try {
      const response = await axios.get(
        "https://api.travelo10.com/api/weekends/weekend-national"
      );
  ...
  const fetchChosenNationalData = async () => {
    try {
      const response = await axios.get(
        "https://api.travelo10.com/api/weekends/weekend-choosen"
      );
  ...
  const handleChoosePackage = async (trip, stateId) => {
    try {
      await axios.post("https://api.travelo10.com/api/weekends/weekend", {
        entryId: trip._id,
        stateId: stateId,
  ...
  const handleDeletePackage = async (packageId) => {
    try {
      await axios.delete(
        `https://api.travelo10.com/api/weekends/weekend/${packageId}`
      );

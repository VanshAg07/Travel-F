...
function AddWeekend() {
  ...
  const fetchNationalData = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/api/weekends/weekend-national"
      );
  ...
  const fetchChosenNationalData = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/api/weekends/weekend-choosen"
      );
  ...
  const handleChoosePackage = async (trip, stateId) => {
    try {
      await axios.post("http://localhost:5000/api/weekends/weekend", {
        entryId: trip._id,
        stateId: stateId,
  ...
  const handleDeletePackage = async (packageId) => {
    try {
      await axios.delete(
        `http://localhost:5000/api/weekends/weekend/${packageId}`
      );

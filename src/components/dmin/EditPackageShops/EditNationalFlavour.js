...
function EditNationalFlavour() {
  ...
  const fetchStates = () => {
    setLoading(true);
    axios
      .get("https://api.travelo10.com/api/admin/getFlavour")
      .then((response) => {
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .put(
        `https://api.travelo10.com/api/admin/editFlavour/${selectedActivity._id}`,
        formData,
        {
          headers: {
  ...
  const handleDelete = (activity) => {
    try {
      axios
        .delete(
          `https://api.travelo10.com/api/admin/deleteActivity/${activity._id}`
        )
        .then((response) => {
...
export default EditNationalFlavour;
